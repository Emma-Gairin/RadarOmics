  ...
  for (cat in catlist){
    ...
    if (n_features > 4){
      ...
      if (num_pcs==1){
        ...
        if (any(constantpc)) {
          message(
            "The following PCs have near-zero variance within your target groups: ",
            paste(colnames(pc_mat)[bad_pcs], collapse = ", "),
            "This can happen when using remove_batch_effect.",
            "These PCs will be removed before running the LDA."
          )
        ...
        prep_mean_group = as.data.frame(predict(sample_lda)$x)
        lda[[cat]]= prep_mean_group
        prep_mean_group$sample = rownames(prep_mean_group)
        ...
        # Calculate the variance and cumulative variance corresponding to each dimension of the LDA
        explained_variance = sample_lda$svd^2
        proportion_variance = explained_variance/sum(explained_variance)
        cumulative_variance = cumsum(proportion_variance)
        ...
        if (num_lds==1){
          ...
          coordinate_mainaxis = data.frame(distance = prep_mean_group[,"LD1"])
          rownames(coordinate_mainaxis) = rownames(prep_mean_group)
          centroid = NA
          maxvariancedirection=NA
        } else {
          centroid = colMeans(mean_group[,paste0("LD",1:num_lds)])
          covariancematrix = cov(mean_group[,paste0("LD",1:num_lds)])
          eigen_data_input = eigen(covariancematrix)
          maxvariancedirection = eigen_data_input$vectors[,1]
          projected_points = t(apply(prep_mean_group[,paste0("LD",1:num_lds)],1,projection_multidim,
      ...
      mean_group_dim = aggregate(  as.formula(paste(". ~", focus)),coordinate_mainaxis[,c("distance",focus)],mean)
      colnames(mean_group_dim)=c("group","distance")
      furthest_groups = paste0(
        mean_group_dim$group[which.min(mean_group_dim$distance)],
        "-",
